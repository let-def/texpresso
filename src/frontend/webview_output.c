...
{
  if (state->tmpdir) free(state->tmpdir);
  state->tmpdir = dir ? strdup(dir) : NULL;
  if (dir && !state->tmpdir)
    fprintf(stderr, "[webview] WARNING: strdup failed for tmpdir\n");
}
                         ...
                         bool dark_mode, float trim_factor)
{
  ...
  if (*prev_rgb && *prev_w == w && *prev_h == h && *prev_page == page) {
    ...
    if (n_rects == 0) {
      ...
      struct { int x, y, w, h; char path[WEBVIEW_PATH_MAX]; } emitted[MAX_DIRTY_RECTS];
      int emitted_count = 0;
      for (int i = 0; i < n_rects; i++) {
        ...
        strncpy(emitted[emitted_count].path, rpath, WEBVIEW_PATH_MAX - 1);
        emitted[emitted_count].path[WEBVIEW_PATH_MAX - 1] = '\0';
        emitted_count++;
      }
