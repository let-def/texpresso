...
\usepackage{tikz}
\usetikzlibrary{arrows.meta, shapes, calc, backgrounds, positioning, patterns, decorations.markings, decorations.pathmorphing, fit}
\usepackage{xcolor}

\begin{document}

\section*{TeXpresso TikZ 支持功能测试}

\subsection*{Phase 0: Dash 模式 (长 dash pattern > 4 元素)}
...
\subsection*{Phase 1: 填充和描边}

\begin{tikzpicture}
  % fill only
  \fill[red!50] (0,0) circle (0.8);
  % draw only
...
\subsection*{Phase 1: 线型 (line width, join, cap, dash)}
...
\subsection*{Phase 1: 图形状态 (q/Q, cm, W)}
...
\subsection*{Phase 1: 路径操作符 (m, l, c, v, y, h, re)}
...
\subsection*{Phase 5: 标记内容 (BMC/BDC/EMC — no-op) 和装饰}

\begin{tikzpicture}
  % BMC/BDC/EMC 测试: 带 layers(tikz 内部使用)
  % decorations.markings (pathmorphing 也可能使用标记内容)
