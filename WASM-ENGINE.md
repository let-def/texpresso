...
| 3.3 | Fence stack (fork-engine-equivalent incremental) | **done — layered userland COW (`snapshot_push`/`restore_to`, growth-aware, demand-zero shadows) + a fence every ~32KB of document. An edit replays from the deepest fence before it, byte-identical to a full re-run at 10/50/90%. Standalone A/B + fence memory self-tests pass.** |
| 4 | **Retire the fork; standalone wasm-only** | **done — wasm is the sole TeX engine, renamed to `engine_tex` (keeps the .fmt role name). Fork driver + patched native engine (`src/engine`) deleted; build needs only the wasm2c engine. Fork fence/rollback algorithm preserved verbatim in `fence_ref.c` for a later fine-grained port. `Q_SEEN` shown recoverable without patching via read-clipping (xetex reads 1KB, re-requests on short reads). `sprotocol`, editor protocol, pdf/dvi viewers, `-texlive`/`-tectonic` display hooks untouched.** |
