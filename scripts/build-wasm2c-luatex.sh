#!/bin/bash
# wasm2c the luatex wasm and link a native (JS-free) engine.
#
# Same recipe as build-wasm2c-pdftex.sh with `-n engine`, so the identical host
# (src/engine-wasm/wasm_host.c) drives luatex. The generated engine.c is large
# (~180 MB) and slow to compile.
#
# Prerequisite: scripts/build-wasm-luatex.sh.
# Output: engines/build-wasm2c-luatex/luatex-native
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
WASM="$ROOT/engines/build-wasm-luatex/texk/web2c/luatex.wasm"
OUT="$ROOT/engines/build-wasm2c-luatex"
HOST="$ROOT/src/engine-wasm/wasm_host.c"

[ -f "$WASM" ] || { echo "missing $WASM — run build-wasm-luatex.sh first" >&2; exit 1; }
